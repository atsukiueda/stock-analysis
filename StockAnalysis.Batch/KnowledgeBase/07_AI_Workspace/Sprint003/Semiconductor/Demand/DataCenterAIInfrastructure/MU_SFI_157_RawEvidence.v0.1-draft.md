# Micron Technology — MU-SFI-157 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-157` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-139` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Broadcom relation、qualification、adoption、procurement、shipment又はrevenueを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-07-29 SSD release, 9650 section |
...
Micronは9650 SSDについてAstera Labsとのprior public demonstrationによるinteroperability relationを説明した。

| Field | Value |
| --- | --- |
| Fact type / classification | Ecosystem-interoperability relationship — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Broadcom edge `158` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Broadcom relation、qualification、adoption、procurement、shipment又はrevenueを補完しない。
