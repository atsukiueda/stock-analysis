# Micron Technology — MU-SFI-126 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-126` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-113` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Price、volume、revenue、delivery又はForecast achievementを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: calendar 2026 |
...
Micronはcalendar 2026 HBM3E supplyのvast majorityについてalmost all customersとpricing agreementsがあると説明する。

| Field | Value |
| --- | --- |
| Fact type / classification | Pricing-agreement assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | HBM4 discussions `127` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Price、volume、revenue、delivery又はForecast achievementを補完しない。
