# Micron Technology — MU-SFI-051 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-051` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-043` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Application、completed qualification、customer identity / count、order、shipment、adoption又はrevenueを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。

## 1. 一次資料

| 項目 | 内容 |
| --- | --- |
| 資料名 | Micron Technology, Inc., *Micron FY2023 Form 10-K* |
| Publication Event | 2023-10-06; time / timezone Unknown |
| Applicable Period | Publication Event 2023-10-06時点のcurrent / Actual assertion |
| URL | [Official source](https://investors.micron.com/sec-filings/sec-filing/10-k/0000723125-23-000054) |
| Source position | FY2023 10-K printed p.9 / physical p.10 / viewer P9, SSD qualification |
...
当該Data Center SSDは他のcustomerでqualification進行中と説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Anonymous-customer ongoing-qualification assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Application `116`; completed qualification `012` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Application、completed qualification、customer identity / count、order、shipment、adoption又はrevenueを補完しない。
