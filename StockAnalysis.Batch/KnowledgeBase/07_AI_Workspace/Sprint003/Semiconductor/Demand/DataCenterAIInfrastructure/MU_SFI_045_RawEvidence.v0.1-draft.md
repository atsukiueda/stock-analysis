# Micron Technology — MU-SFI-045 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-045` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-039` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Development status、Actual、customer commitment又はForecast revenueではない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: calendar 2027 |
...
1-gamma DRAM技術のHBM4Eはcalendar 2027 volume production見込み。

| Field | Value |
| --- | --- |
| Fact type / classification | Volume-production Plan — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Development status is separate `067` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Development status、Actual、customer commitment又はForecast revenueではない。
