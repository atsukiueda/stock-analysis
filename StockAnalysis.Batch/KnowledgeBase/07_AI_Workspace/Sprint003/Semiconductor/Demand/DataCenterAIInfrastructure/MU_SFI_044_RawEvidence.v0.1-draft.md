# Micron Technology — MU-SFI-044 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-044` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-038` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Qualification完了、mass production、adoption、order又はrevenueにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
HBM4 qualification samplesを複数end-customerへshipmentした。

| Field | Value |
| --- | --- |
| Fact type / classification | Qualification-sample shipment assertion — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | Same event/product as `043`, distinct stage / party grain |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Qualification完了、mass production、adoption、order又はrevenueにしない。
