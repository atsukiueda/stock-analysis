# Micron Technology — MU-SFI-020 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-020` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-019` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Application、qualification、mass production、shipment又はrevenueへ昇格しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: FY2024 |
| URL | [Official source](https://investors.micron.com/sec-filings/sec-filing/10-k/0000723125-24-000027) |
| Source position | FY2024 10-K printed p.9 / physical p.10 / viewer P9, 9550 SSD |
...
FY2024に9550 series SSDのsamplingを開始した。

| Field | Value |
| --- | --- |
| Fact type / classification | Product sampling Actual — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | Application is separate `054`; later qualification `030` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Application、qualification、mass production、shipment又はrevenueへ昇格しない。
