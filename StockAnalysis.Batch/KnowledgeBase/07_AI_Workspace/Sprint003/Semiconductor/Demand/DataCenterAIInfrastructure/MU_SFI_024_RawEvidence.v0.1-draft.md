# Micron Technology — MU-SFI-024 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-024` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-023` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** 各製品金額、share、unit又はAI-only mixを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | FY2025 10-K p.10, CMBU sales |
...
FY2025 CMBU Data Center salesは主にHBM、DDR5 / DDR4、LPDDR5及びGDDR6で構成された。

| Field | Value |
| --- | --- |
| Fact type / classification | Actual-period product mix — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Same segment as `023` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- 各製品金額、share、unit又はAI-only mixを補完しない。
