# Micron Technology — MU-SFI-137 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-137` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-124` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Denominatorを保持し、per-system capacity、shipment又はrevenueへ換算しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-10-22 SOCAMM2 release, opening |
...
192GB SOCAMM2はsame compact footprintのearlier SOCAMM比50% higher capacityと説明される。
...
| Related candidate boundary | TTFT `138` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Denominatorを保持し、per-system capacity、shipment又はrevenueへ換算しない。
