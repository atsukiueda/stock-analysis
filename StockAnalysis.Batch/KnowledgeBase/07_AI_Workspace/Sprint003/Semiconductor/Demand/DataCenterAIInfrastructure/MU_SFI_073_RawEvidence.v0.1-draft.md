# Micron Technology — MU-SFI-073 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-073` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-064` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Comparison configurationを保持し、all-server power又はdeployed resultへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
256GB SOCAMM2はstandard RDIMM比約3分の1のpowerとのissuer comparisonを示す。
...
| Related candidate boundary | COMPUTEX `037` is later corroboration; one Fact owner only |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Comparison configurationを保持し、all-server power又はdeployed resultへ一般化しない。
