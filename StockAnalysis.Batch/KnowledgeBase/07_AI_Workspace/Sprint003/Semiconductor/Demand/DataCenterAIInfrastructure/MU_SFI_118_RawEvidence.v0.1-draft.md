# Micron Technology — MU-SFI-118 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-118` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-105` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** B200 relation、procurement、deployment count、shipment又はrevenueを示さない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
Micron HBM3E 8H 24GBはNVIDIA GB200 NVL72 platform向けにavailableと説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Named-platform availability assertion — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | B200 edge `035` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- B200 relation、procurement、deployment count、shipment又はrevenueを示さない。
