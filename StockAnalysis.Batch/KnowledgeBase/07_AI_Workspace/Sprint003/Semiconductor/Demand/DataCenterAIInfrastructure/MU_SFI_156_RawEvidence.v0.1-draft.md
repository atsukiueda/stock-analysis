# Micron Technology — MU-SFI-156 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-156` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-138` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Actual shipment、availability、production、customer又はrevenueにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: calendar Q3 2025 |
...
122TB 6600 ION samplesはcalendar Q3 2025後半にshipping予定と説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Future sample-shipment Plan — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Introduction `131`; later events are separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Actual shipment、availability、production、customer又はrevenueにしない。
