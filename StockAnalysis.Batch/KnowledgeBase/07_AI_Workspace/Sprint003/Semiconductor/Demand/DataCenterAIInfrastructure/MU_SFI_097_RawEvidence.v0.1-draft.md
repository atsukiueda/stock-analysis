# Micron Technology — MU-SFI-097 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-097` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-085` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Supply確保、Micron product adoption、order、shipment、revenue、demand quantity又はForecastへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2026-06-22 release, Anthropic speaker statement |
...
Anthropic speakerはAI infrastructureにおけるmemory / storageの中心性について述べた。

| Field | Value |
| --- | --- |
| Fact type / classification | External-party assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Supply assertion `120` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Supply確保、Micron product adoption、order、shipment、revenue、demand quantity又はForecastへ変換しない。
