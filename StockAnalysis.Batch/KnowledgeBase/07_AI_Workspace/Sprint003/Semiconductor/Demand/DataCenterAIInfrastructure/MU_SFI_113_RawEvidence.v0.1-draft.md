# Micron Technology — MU-SFI-113 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-113` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-100` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Configuration denominatorを保持し、installed capacity、shipment、unit又はrevenueへ換算しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Publication Event 2026-03-03 09:00 EST時点のcurrent / Actual assertion |
...
| Source position | 2026-03-03 release, product paragraph |
| Retrieval Date | 2026-08-13 |
| Corroborating provenance | Micron in High-Volume Production of HBM4 Designed for NVIDIA Vera Rubin, PCIe Gen6 SSD and SOCAMM2 (2026-03-16 17:00 EDT), https://investors.micron.com/news-releases/news-release-details/micron-high-volume-production-hbm4-designed-nvidia-vera-rubin — corroboration-only |

## 2. Source Fact

Source-defined 256GB SOCAMM2 configurationは8-channel CPU当たり2TB memoryを可能にすると説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Per-CPU capacity assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Later corroboration `109`; module comparison `077` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Configuration denominatorを保持し、installed capacity、shipment、unit又はrevenueへ換算しない。
