# Micron Technology — MU-SFI-150 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-150` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-132` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Introduction、availability、qualification、shipment、adoption又はrevenueへ昇格しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-07-29 SSD release, 7600 highlight |
...
Micron 7600 PCIe Gen5 SSDはmainstream Data Center workload向けと位置付けられる。

| Field | Value |
| --- | --- |
| Fact type / classification | Product / application positioning — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Introduction `133` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Introduction、availability、qualification、shipment、adoption又はrevenueへ昇格しない。
