# Micron Technology — MU-SFI-121 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-121` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-108` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Key OEM class、party identity / count、qualification完了、order、shipment、adoption又はrevenueを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
Micron 9650はAI Data Center customersによりqualification中と説明される。
...
| Related candidate boundary | Key OEM class `115` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Key OEM class、party identity / count、qualification完了、order、shipment、adoption又はrevenueを補完しない。
