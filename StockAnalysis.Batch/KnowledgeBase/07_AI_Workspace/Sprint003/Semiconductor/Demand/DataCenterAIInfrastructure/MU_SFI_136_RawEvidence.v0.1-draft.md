# Micron Technology — MU-SFI-136 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-136` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-123` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Sampling、deployment、adoption、shipment量又はrevenueにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Publication Event 2025-10-22 09:02 EDT時点のproduct / application positioning assertion |
...
192GB SOCAMM2をAI Data Center low-power memory向けと位置付ける。
...
| Related candidate boundary | Stage `135` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Sampling、deployment、adoption、shipment量又はrevenueにしない。
