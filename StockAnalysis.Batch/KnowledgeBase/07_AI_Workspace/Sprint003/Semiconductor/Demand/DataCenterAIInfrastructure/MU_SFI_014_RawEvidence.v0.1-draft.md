# Micron Technology — MU-SFI-014 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-014` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-014` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Customer、shipment volume、revenue、share又はAI adoptionを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | FY2024 10-K p.7, HBM3E |
...
FY2024に8-high 24GB HBM3Eのvolume productionを開始した。

| Field | Value |
| --- | --- |
| Fact type / classification | Mass-production commencement Actual — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | Actual following `009` Plan |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Customer、shipment volume、revenue、share又はAI adoptionを補完しない。
