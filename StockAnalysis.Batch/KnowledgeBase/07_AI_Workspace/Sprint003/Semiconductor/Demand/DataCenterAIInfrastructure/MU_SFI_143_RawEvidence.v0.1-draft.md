# Micron Technology — MU-SFI-143 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-143` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-130` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Customer identity、product、quantity、price、shipment又はrevenueを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | Q3 FY2026 earnings deck, Overview |
...
Micronは16 strategic customer agreementsを締結したと説明する。

| Field | Value |
| --- | --- |
| Fact type / classification | Strategic-customer-agreement count assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Agreement count, not order count |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Customer identity、product、quantity、price、shipment又はrevenueを補完しない。
