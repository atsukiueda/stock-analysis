# Micron Technology — MU-SFI-111 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-111` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-098` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Tableのtest / configuration basisを保持し、all workload又はdeployed performanceへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Publication Event 2026-02-11時点のsequential-read throughput assertion（Fact本文の条件・単位に限定） |
...
Micron 9650はsource-defined configurationで最大28 GB/s sequential read throughputを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Sequential-read-throughput assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Random IOPS `112` separate; Mar16 corroborates |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Tableのtest / configuration basisを保持し、all workload又はdeployed performanceへ一般化しない。
