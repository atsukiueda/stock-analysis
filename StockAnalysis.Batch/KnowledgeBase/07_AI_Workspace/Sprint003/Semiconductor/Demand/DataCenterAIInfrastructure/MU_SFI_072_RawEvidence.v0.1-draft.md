# Micron Technology — MU-SFI-072 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-072` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-063` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Sampling、deployment、customer adoption、shipment量又はrevenueにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2026-03-03 release, opening |
...
256GB SOCAMM2をAI / HPC及びData Center infrastructure向けと位置付ける。
...
| Related candidate boundary | Sample stage `071` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Sampling、deployment、customer adoption、shipment量又はrevenueにしない。
