# Micron Technology — MU-SFI-061 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-061` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-052` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Deployed performance、all configurations、relative speed、shipment量又はrevenueへ拡張しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Publication Event 2026-05-12 09:01 EDT時点のcurrent / Actual assertion |
...
256GB DDR5 RDIMMは最大9,200 MT/sと説明される。
...
- Deployed performance、all configurations、relative speed、shipment量又はrevenueへ拡張しない。
