# Micron Technology — MU-SFI-074 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-074` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-065` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Comparison configurationを保持し、rack footprint又はdeployment densityへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2026-03-03 release, comparison paragraphs |
...
256GB SOCAMM2はstandard RDIMM比約3分の1のfootprintとのissuer comparisonを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Comparative-footprint assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | COMPUTEX `060` is later corroboration; one Fact owner only |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Comparison configurationを保持し、rack footprint又はdeployment densityへ一般化しない。
