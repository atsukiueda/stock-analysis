# Micron Technology — MU-SFI-086 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-086` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-077` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Procurement、deployment、shipment量、adoption又はrevenueへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
Micron 192GB SOCAMM2はNVIDIA Vera Rubin向けrelationを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Named-platform relationship — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Stage `085` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Procurement、deployment、shipment量、adoption又はrevenueへ変換しない。
