# Micron Technology — MU-SFI-167 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-167` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-149` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Deployed throughput、all configuration、customer result又はrevenueへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-10-22 SOCAMM2 release, Availability |
...
SOCAMM2 customer samplesは最大9.6 Gbpsと説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Product-speed assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Sampling `135`; performance only |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Deployed throughput、all configuration、customer result又はrevenueへ一般化しない。
