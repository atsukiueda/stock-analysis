# Micron Technology — MU-SFI-003 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-003` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-003` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Availability、adoption、shipment又はcustomer resultではない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。

## 1. 一次資料

| 項目 | 内容 |
| --- | --- |
| 資料名 | Micron Technology, Inc., *Micron FY2022 Form 10-K* |
| Publication Event | 2022-10-07; time / timezone Unknown |
| Applicable Period | Publication Event 2022-10-07時点の将来表明（Fact本文記載の時期・条件に限定） |
| URL | [Official source](https://investors.micron.com/sec-filings/sec-filing/10-k/0000723125-22-000048) |
| Source position | FY2022 10-K p.8, Cloud Server |
...
Server DDR5のbandwidthがAI / HPC等のdata-intensive workloadのsystem performanceを支えるとの将来expectationを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Future product / system expectation — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Different grain from `001` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Availability、adoption、shipment又はcustomer resultではない。
