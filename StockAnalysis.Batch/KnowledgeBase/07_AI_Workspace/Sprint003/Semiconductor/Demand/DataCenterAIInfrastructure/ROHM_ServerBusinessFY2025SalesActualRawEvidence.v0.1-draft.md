# ROHM — Server Business FY2025 Sales Actual Raw Evidence
...
| Issuer | ROHM Co., Ltd. |
| Inspection Source | `ROHMSourceFactInspection.v0.1-draft.md` — `ROHM-SFI-010` |
| Cross-sprint Recheck | `ROHMCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical candidate |
| Evidence ID | `S3-EVR-050` |
| Evidence Registration Status | Registered — `Sprint003DataCenterAIInfrastructureEvidenceRegister.v0.1-draft.md` / `S3-EVR-050`; EVR Review Accepted |
| PIT Mapping Status | Registered — `Sprint003DataCenterAIInfrastructurePITInventory.v0.1-draft.md` / `S3-PIT-050`; PIT Review Accepted |

> **利用境界：** `Server Business`はAI server単独又はData Center単独と定義されていない。C&S segment又はAI server revenueと同一視しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
FY2025 Server Business salesはJPY17.0bnと表示される。

| Field | Value |
| --- | --- |
| Fact type | Source-presented Actual |
| Actual / Forecast | Source-presented Actual — 上流Inspectionの分類を変更しない |
| Unit | JPY billion |
...
- `Server Business`はAI server単独又はData Center単独と定義されていない。C&S segment又はAI server revenueと同一視しない。
