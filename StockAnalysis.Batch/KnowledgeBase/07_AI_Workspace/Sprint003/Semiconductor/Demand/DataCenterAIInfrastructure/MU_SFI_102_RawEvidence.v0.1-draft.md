# Micron Technology — MU-SFI-102 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-102` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-090` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Hopper relation、platform-wide procurement、customer count、shipment volume、revenue又はexclusive adoptionへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
Micron HBM3EについてNVIDIA Blackwell platform deployment relationを説明する。
...
| Related candidate boundary | Hopper is separate `099` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Hopper relation、platform-wide procurement、customer count、shipment volume、revenue又はexclusive adoptionへ変換しない。
