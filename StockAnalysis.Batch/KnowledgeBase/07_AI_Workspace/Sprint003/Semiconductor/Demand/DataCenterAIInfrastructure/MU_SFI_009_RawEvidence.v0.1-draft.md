# Micron Technology — MU-SFI-009 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-009` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-009` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** PlanでありActualではない。`014`の後続Actualへ遡及変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: early calendar 2024 |
...
MicronはHBM3E mass-production rampをearly calendar 2024に開始する計画を示した。

| Field | Value |
| --- | --- |
| Fact type / classification | Mass-production Plan — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Historical Plan preceding `014` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- PlanでありActualではない。`014`の後続Actualへ遡及変換しない。
