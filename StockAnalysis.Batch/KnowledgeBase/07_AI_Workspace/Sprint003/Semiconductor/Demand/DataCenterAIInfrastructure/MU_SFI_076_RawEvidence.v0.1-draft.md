# Micron Technology — MU-SFI-076 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-076` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-067` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Test basisを保持し、system / rack power又はcommercial resultへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
Source-defined stand-alone CPU testで256GB SOCAMM2はcomparison configuration比3倍のperformance per wattを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Internal test / performance-per-watt assertion — 上流Inspectionの分類を変更しない |
...
- Test basisを保持し、system / rack power又はcommercial resultへ変換しない。
