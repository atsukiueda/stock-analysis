# Micron Technology — MU-SFI-131 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-131` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-118` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** 245TB identity、shipment、availability又はrevenueへ拡張しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-07-29 SSD release, 6600 ION highlight |
...
MicronはG9 NAND採用6600 ION E3.S 122TB Data Center SSDを発表した。

| Field | Value |
| --- | --- |
| Fact type / classification | Product introduction assertion — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | 245TB Plan `132` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- 245TB identity、shipment、availability又はrevenueへ拡張しない。
