# Micron Technology — MU-SFI-103 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-103` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-091` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Tableのworkload / power-state basisを保持し、random readを含むall-workload efficiency、system / rack efficiency、TCO又はcustomer resultへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2026-02-11 official product blog, efficiency table |
...
Micron 9650はsource-defined Gen5 comparisonに対して100% higher sequential read performance per wattを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Comparative-sequential-read-performance-per-watt assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Sequential-read comparison `084` separate; Mar16 corroborates |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Tableのworkload / power-state basisを保持し、random readを含むall-workload efficiency、system / rack efficiency、TCO又はcustomer resultへ一般化しない。
