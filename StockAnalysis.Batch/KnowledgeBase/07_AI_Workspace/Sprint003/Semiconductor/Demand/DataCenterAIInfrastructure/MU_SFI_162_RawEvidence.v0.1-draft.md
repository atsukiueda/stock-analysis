# Micron Technology — MU-SFI-162 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-162` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-144` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Production deployment、qualification、procurement、shipment又はrevenueにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-07-29 SSD release, Astera Labs speaker statement |
...
Astera Labs speakerはMicron 9650、Scorpio P-Series Fabric Switch及びAries PCIe Gen6 Retimerを接続するdemonstrated interoperabilityについて述べた。

| Field | Value |
| --- | --- |
| Fact type / classification | External-party interoperability assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Issuer relation `157` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Production deployment、qualification、procurement、shipment又はrevenueにしない。
