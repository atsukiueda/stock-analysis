# Micron Technology — MU-SFI-041 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-041` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-035` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** HBM / AI-only revenueへ配賦しない。Fiscal-quarter denominatorを保持する。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: FQ3-26 / FQ2-26 / FQ3-25 |
| URL | [Official source](https://investors.micron.com/node/50671) |
| Source position | Q3 FY2026 release, Business Unit Results |
...
FQ3-26 CMBU revenueはUSD13.769bn、FQ2-26 USD7.749bn、FQ3-25 USD3.386bn。

| Field | Value |
| --- | --- |
| Fact type / classification | Segment revenue Actual — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | Same event as `042` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- HBM / AI-only revenueへ配賦しない。Fiscal-quarter denominatorを保持する。
