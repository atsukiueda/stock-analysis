# Micron Technology — MU-SFI-117 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-117` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-104` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** B300 relation、procurement、adoption、shipment volume又はrevenueにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
Micron HBM3E 12H 36GBはNVIDIA GB300 NVL72 platformにdesigned intoされている。

| Field | Value |
| --- | --- |
| Fact type / classification | Named-platform designed-into relationship — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | B300 edge `034` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- B300 relation、procurement、adoption、shipment volume又はrevenueにしない。
