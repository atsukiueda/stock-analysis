# Micron Technology — MU-SFI-077 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-077` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-068` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Comparison denominatorを保持し、per-CPU capacity、unit、installed base、shipment又はrevenueへ換算しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
256GB SOCAMM2はcomparison module比1.33倍のcapacityを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Comparative module-capacity assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Per-CPU capacity is `113` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Comparison denominatorを保持し、per-CPU capacity、unit、installed base、shipment又はrevenueへ換算しない。
