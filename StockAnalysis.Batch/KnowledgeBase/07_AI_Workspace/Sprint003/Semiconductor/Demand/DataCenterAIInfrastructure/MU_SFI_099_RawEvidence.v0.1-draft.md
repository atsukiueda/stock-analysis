# Micron Technology — MU-SFI-099 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-099` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-087` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Blackwell relation、platform-wide procurement、customer count、shipment volume、revenue又はexclusive adoptionへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Publication Event 2025-03-18 16:24 EDT時点のcurrent / Actual assertion |
...
| Source position | 2025-03-18 release, HBM paragraph |
...
Micron HBM3EについてNVIDIA Hopper platform deployment relationを説明する。

| Field | Value |
| --- | --- |
| Fact type / classification | Named-platform deployment relationship — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Blackwell is separate `102` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Blackwell relation、platform-wide procurement、customer count、shipment volume、revenue又はexclusive adoptionへ変換しない。
