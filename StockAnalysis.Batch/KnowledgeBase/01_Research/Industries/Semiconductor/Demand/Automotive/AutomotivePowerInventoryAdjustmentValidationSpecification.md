...
国内企業について、
EDINET XBRL等からInventory関連項目を取得できるか検討する。
...
ただし、Company Universeは別Researchで正式確定する。
...
内訳項目はCompany-specific Observation候補とする。

企業固有Taxonomy Extensionに依存する項目は、
Production Common Featureへ使用しない候補とする。
...
Existing DB確認
↓
Official IR / EDINET Source Feasibility確認
↓
Inventory Dataset Prototype
↓
Validation
↓
Knowledge Contribution確認
↓
Data Cost Review
```
...
まず取得可能な一次資料・既存DB・EDINET候補からPrototype Dataset構築可能性を調査する。
...
1. 現在のDB Entity / TableにInventory項目が存在するか確認する。
2. J-Quants Summary APIでInventoryが取得できないことを確定する。
3. EDINET / 公式IRからInventory取得可能性をResearchする。
4. ROHMをReference CompanyとしてHistorical Inventory Dataset候補を確認する。
5. Inventory / Revenue算出用Revenue Definitionを確定する。
6. Power Semiconductor Company Universe Researchへ進む。
7. Prototype Dataset設計はCatalog / DDL前には実装しない。
