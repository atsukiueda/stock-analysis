# Micron Technology — MU-SFI-125 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-125` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-112` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Identity、share、shipment、revenue又はadoption scoreを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
MicronのHBM customer baseは6 customersを含むと説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Anonymous-customer-count assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Commercial context, not revenue |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Identity、share、shipment、revenue又はadoption scoreを補完しない。
