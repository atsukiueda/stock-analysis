# Micron Technology — MU-SFI-127 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-127` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-114` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Agreement、order、shipment、production又はrevenueへ昇格しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | FY2025 Q4 prepared remarks, HBM paragraph |
...
MicronはHBM4 specifications及びvolumesについてcustomersとactive discussions中と説明する。

| Field | Value |
| --- | --- |
| Fact type / classification | Customer-discussion assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Future agreement expectation `128` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Agreement、order、shipment、production又はrevenueへ昇格しない。
