# Micron Technology — MU-SFI-005 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-005` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-005` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** CNBU全体又はAI-only salesとせず、金額を補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
FY2023 cloud-market salesは主にDDR4及びDDR5 DRAMで構成された。
...
| Related candidate boundary | Different from segment revenue |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- CNBU全体又はAI-only salesとせず、金額を補完しない。
