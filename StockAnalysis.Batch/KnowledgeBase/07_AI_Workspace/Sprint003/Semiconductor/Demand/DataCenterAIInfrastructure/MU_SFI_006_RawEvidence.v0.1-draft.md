# Micron Technology — MU-SFI-006 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-006` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-006` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** `002`の独立需要signal又は確度加算にせず、event別provenanceを保持する。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Publication Event 2023-10-06時点で開示されたperiod comparison（Fact本文の比較基準に限定） |
| URL | [Official source](https://investors.micron.com/sec-filings/sec-filing/10-k/0000723125-23-000054) |
| Source position | FY2023 10-K p.7, Cloud Server |
...
AI / data-centric cloud workloadには増加するDRAM、HBM及びNANDが必要とのissuer assertionを再掲する。

| Field | Value |
| --- | --- |
| Fact type / classification | Issuer demand assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Cross-event restatement of demand framing |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- `002`の独立需要signal又は確度加算にせず、event別provenanceを保持する。
