# Micron Technology — MU-SFI-033 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-033` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-030` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** GB300 support、procurement、availability、shipment volume、revenue又はcommercial successへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
SOCAMMはNVIDIAと共同開発されたと説明する。

| Field | Value |
| --- | --- |
| Fact type / classification | Named-party joint-development relationship — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | GB300 support is separate `059` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- GB300 support、procurement、availability、shipment volume、revenue又はcommercial successへ変換しない。
