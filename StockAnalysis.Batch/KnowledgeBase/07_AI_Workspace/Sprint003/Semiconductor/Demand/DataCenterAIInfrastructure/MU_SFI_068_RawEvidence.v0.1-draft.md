# Micron Technology — MU-SFI-068 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-068` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-059` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Individual capacity、availability、shipment、customer、revenue又はshareを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
LP5X SOCAMM2 offeringを複数capacity pointへ拡大した。

| Field | Value |
| --- | --- |
| Fact type / classification | Product-family expansion assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | HVP assertion `047` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Individual capacity、availability、shipment、customer、revenue又はshareを補完しない。
