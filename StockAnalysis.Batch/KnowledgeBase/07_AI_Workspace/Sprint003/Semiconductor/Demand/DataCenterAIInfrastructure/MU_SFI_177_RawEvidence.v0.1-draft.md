# Micron Technology — MU-SFI-177 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-177` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-159` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Module / system / rack saving、deployed result又はuniversal ratioへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Publication Event 2025-10-22 09:02 EDT時点で開示された比較 / test basis（Fact本文の条件に限定） |
| URL | [Official source](https://investors.micron.com/node/49611) |
| Source position | 2025-10-22 SOCAMM2 release, comparison paragraph / footnote 2 |
...
192GB SOCAMM2の1-gamma DRAMはprevious-generation LPDDR5X比20%超のpower-efficiency improvementと説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Comparative-process-efficiency assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Other comparison metrics remain §4.1 exclusions |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Module / system / rack saving、deployed result又はuniversal ratioへ一般化しない。
