# Micron Technology — MU-SFI-123 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-123` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-110` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Test / validation basisを保持し、system throughput又はdeployed performanceへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
Micron HBM4は11 Gb/s超のpin speedを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Pin-speed assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Bandwidth `124` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Test / validation basisを保持し、system throughput又はdeployed performanceへ一般化しない。
