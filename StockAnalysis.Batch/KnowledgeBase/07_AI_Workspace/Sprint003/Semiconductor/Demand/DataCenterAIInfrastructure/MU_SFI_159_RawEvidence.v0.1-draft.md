# Micron Technology — MU-SFI-159 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-159` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-141` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Deployment Actual、customer adoption、order、shipment quantity、revenue又はuniversal configurationへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-07-29 SSD release, Supermicro speaker statement |
...
Supermicro speakerは6600 IONが同社storage serverで高密度AI workloadを支えるとの見解を述べた。

| Field | Value |
| --- | --- |
| Fact type / classification | External-party product / application assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | External assertion |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Deployment Actual、customer adoption、order、shipment quantity、revenue又はuniversal configurationへ変換しない。
