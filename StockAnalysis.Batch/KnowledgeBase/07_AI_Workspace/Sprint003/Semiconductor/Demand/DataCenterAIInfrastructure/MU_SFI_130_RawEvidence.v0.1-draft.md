# Micron Technology — MU-SFI-130 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-130` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-117` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Mass production、availability、shipment又はrevenueへ昇格しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-07-29 SSD release, 9650 highlight |
...
Micronは9650 PCIe Gen6 Data Center SSDを発表した。
...
| Related candidate boundary | Later mass production `114` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Mass production、availability、shipment又はrevenueへ昇格しない。
