# Micron Technology — MU-SFI-056 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-056` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-048` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Qualification、quantity、customer、AI-only revenue又はadoption rateを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: FY2025 |
| URL | [Official source](https://investors.micron.com/sec-filings/sec-filing/10-k/0000723125-25-000028) |
| Source position | FY2025 10-K printed p.10 / physical p.11 / viewer P10, 9550 SSD |
...
FY2025に9550 series SSDのAI / HPC等workload向けshipmentを開始した。

| Field | Value |
| --- | --- |
| Fact type / classification | Shipment-commencement Actual — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | Qualification `030`; application `054` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Qualification、quantity、customer、AI-only revenue又はadoption rateを補完しない。
