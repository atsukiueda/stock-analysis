# Micron Technology — MU-SFI-089 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-089` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-080` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Shipment quantity、deployment、adoption、revenue又はmarket shareへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2026-05-05 release, application paragraphs |
...
245TB Micron 6600 IONをAI、cloud、enterprise及びhyperscale use向けと位置付ける。
...
| Related candidate boundary | Stage `088` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Shipment quantity、deployment、adoption、revenue又はmarket shareへ変換しない。
