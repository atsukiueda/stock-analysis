# Micron Technology — MU-SFI-139 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-139` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-126` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** CMBU / CDBU又はHBM / SSDへの配賦、annualized run rate、Forecast又は通期revenueへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: Q3 FY2026 |
...
Fiscal Q3 FY2026 Data Center revenueはUSD25bn超と説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Fiscal-quarter revenue Actual — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Run-rate assertion `152`; SSD Actual `140` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- CMBU / CDBU又はHBM / SSDへの配賦、annualized run rate、Forecast又は通期revenueへ変換しない。
