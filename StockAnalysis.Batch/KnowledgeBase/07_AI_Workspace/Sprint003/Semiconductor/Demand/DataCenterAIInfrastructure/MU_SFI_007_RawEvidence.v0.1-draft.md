# Micron Technology — MU-SFI-007 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-007` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-007` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Quantity、share、customer、order又はrevenueへ変換しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | FY2023 10-K p.7, HBM |
...
FY2023のHBM demandはgenerative AI demandによりvery strongだったとMicronは説明する。

| Field | Value |
| --- | --- |
| Fact type / classification | Actual-period demand assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Same passage as `008` / `009`, separate Fact |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Quantity、share、customer、order又はrevenueへ変換しない。
