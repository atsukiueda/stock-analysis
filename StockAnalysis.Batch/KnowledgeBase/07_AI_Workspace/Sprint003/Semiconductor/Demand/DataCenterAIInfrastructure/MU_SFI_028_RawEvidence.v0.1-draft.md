# Micron Technology — MU-SFI-028 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-028` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-025` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** AI-only、SSD-only又はNAND-only segmentにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | FY2025 10-K p.7, segment definition |
...
CDBUはmid-tier cloud、enterprise、OEM Data Center向けmemory及び全Data Center customer向けstorage solutionsを対象とする。
...
- AI-only、SSD-only又はNAND-only segmentにしない。
