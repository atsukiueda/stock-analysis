...
| Field | Value |
|---|---|
| Document / proposed revision | DOC-CON-000 / DOC-CON-000@1.1.0-draft |
...
| IAP | IAP-DOC-CON-000-1.1.0-DRAFT (Appendix E) |
| Approval record | Pending — no approval record exists for this Draft |
| Release record | Not applicable — this Draft is unreleased |
| Amendment scope | AI Organization and Review Board design; REQ-156–REQ-201; Appendices L–M |
...
For this Revision, the IAP field order is authoritatively extended to `iap-id|profile-version|scope-hash|channel-hash|audience-hash|artifact-type-list|exclusion-list|applicable-requirement-list|as-of`; this sentence supersedes the earlier general IAP-field-order sentence in this appendix. The applicable-requirement-list uses canonical closed-range notation `REQ-001..REQ-201`, which denotes every zero-padded Requirement ID from REQ-001 through REQ-201 inclusive, in ascending ordinal order, with no omissions or additions.

Hash algorithm is SHA-256. Every canonical preimage is a Unicode NFC-normalized sequence encoded as UTF-8 without BOM. Fields are ASCII text only in this Draft. A literal pipe (`|`) separates fields; no whitespace surrounds a pipe; a literal comma (`,`) separates members; no final newline is present. Values are sorted in ascending Unicode code-point order unless an explicit ordinal is shown. `NULL` is the sole null token; an empty string is the sole empty token; a literal backslash is `\\`; a literal pipe is `\\|`; a literal comma is `\\,`; a literal line break is `\\n`. No other escaping is permitted. Snapshot fields, in this exact order, are `snapshot-id|version|member-1|...|member-n|as-of`. IAP fields, in this exact order, are `iap-id|profile-version|scope-hash|channel-hash|audience-hash|artifact-type-list|exclusion-list|as-of`.
...
The initiating team identifies whether work is material, selects the primary subject-matter reviewer, selects a second documentation/governance/editorial reviewer, and records both selections and conflict declarations in the Review Record. An authoring participant may answer findings and revise work, but may not independently approve its own output. A reviewer verifies dispositions and may recommend that a gate be blocked. An authorized approver and Release Authority continue to act under Sections 6 through 8; Review Board participation does not merge those roles. Any participant may escalate an unresolved conflict or a material governance concern to the Project Director.
...
Prior drafts are preserved. v1.1.0 adds the AI Organization and Review Board, including role/profile responsibilities, cross-review, conflict declarations, and extensibility. IAP snapshot hashes were computed using UTF-8 SHA-256 canonical strings and are not pending. This file remains a Draft outside Project Files.
