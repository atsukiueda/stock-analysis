# Micron Technology — MU-SFI-066 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-066` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-057` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** HDD / SSD configuration assumptionsを保持し、Actual customer power、TCO又はuniversal efficiencyへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
245TB Micron 6600 ION SSDはsource-defined HDD-based configuration比でpower半減とのillustrationを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Comparative-power illustration — 上流Inspectionの分類を変更しない |
...
- HDD / SSD configuration assumptionsを保持し、Actual customer power、TCO又はuniversal efficiencyへ一般化しない。
