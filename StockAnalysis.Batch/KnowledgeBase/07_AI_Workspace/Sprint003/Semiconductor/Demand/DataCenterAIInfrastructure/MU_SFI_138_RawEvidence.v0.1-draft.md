# Micron Technology — MU-SFI-138 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-138` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-125` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Test basisを保持し、universal workload又はcustomer deploymentへ一般化しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-10-22 SOCAMM2 release, opening / footnote 1 |
...
Source-defined Llama 3 70B / GH200 NVL2 / LMCache testで192GB SOCAMM2はcomparison configuration比80%超のTTFT reductionを示す。

| Field | Value |
| --- | --- |
| Fact type / classification | Comparative-TTFT assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Capacity `137` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Test basisを保持し、universal workload又はcustomer deploymentへ一般化しない。
