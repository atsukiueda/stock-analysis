# Micron Technology — MU-SFI-067 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-067` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-058` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Volume-production Plan、sample、qualification、shipment又はavailabilityにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
1-gamma DRAM技術のHBM4Eはdevelopment中である。

| Field | Value |
| --- | --- |
| Fact type / classification | Product-development Actual — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | CY2027 Plan `045` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Volume-production Plan、sample、qualification、shipment又はavailabilityにしない。
