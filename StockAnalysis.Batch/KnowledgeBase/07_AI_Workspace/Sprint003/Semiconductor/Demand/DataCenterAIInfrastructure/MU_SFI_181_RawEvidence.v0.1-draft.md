# Micron Technology — MU-SFI-181 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-181` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-163` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Rack footprint、capacity、shipment又はrevenueへ換算しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Source position | 2025-10-22 SOCAMM2 release, comparison paragraph / footnote 5 |
...
SOCAMM2 module areaはstandard server RDIMM比3分の1と説明される。

| Field | Value |
| --- | --- |
| Fact type / classification | Comparative-module-area assertion — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Power comparison `180` separate |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Rack footprint、capacity、shipment又はrevenueへ換算しない。
