# Micron Technology — MU-SFI-047 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-047` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-041` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Capacity-point expansion、unit、customer、AI-only shipment、revenue又はshareを補完しない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
LP5X SOCAMM2 productsはhigh-volume production中である。

| Field | Value |
| --- | --- |
| Fact type / classification | Product-family high-volume-production assertion — 上流Inspectionの分類を変更しない |
| Upstream disposition | Proceed |
| Cross-sprint reference | No identical Source identity / event / position / Fact in Accepted recheck |
| Related candidate boundary | Capacity expansion is separate `068` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Capacity-point expansion、unit、customer、AI-only shipment、revenue又はshareを補完しない。
