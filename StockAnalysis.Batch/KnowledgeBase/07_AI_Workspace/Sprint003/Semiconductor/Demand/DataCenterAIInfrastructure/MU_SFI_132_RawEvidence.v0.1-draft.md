# Micron Technology — MU-SFI-132 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-132` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-119` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** Actual availability、shipment、production、customer又はrevenueにしない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
| Applicable Period | Source Factに明示された期間 / 時点: calendar H1 2026 |
...
245TB 6600 IONはcalendar H1 2026にavailableとなるPlanと説明された。

| Field | Value |
| --- | --- |
| Fact type / classification | Future availability Plan — 上流Inspectionの分類を変更しない |
...
| Related candidate boundary | Later availability owner `122`; shipment `088` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- Actual availability、shipment、production、customer又はrevenueにしない。
