# Micron Technology — MU-SFI-035 Raw Evidence
...
| Inspection Source | `MicronSourceFactInspection.v0.1-draft.md` — `MU-SFI-035` |
| Cross-sprint Recheck | `MicronCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical Fact |
| Evidence / PIT ID | `S3-P2-EVR-032` issued — EVR Review Accepted / PIT not issued |

> **利用境界：** GB200 relation、procurement、deployment count、shipment又はrevenueを示さない。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
Micron HBM3E 8H 24GBはNVIDIA HGX B200 platform向けにavailableと説明される。
...
| Related candidate boundary | GB200 edge `118` |
| AvailableAt | `TBD — no use` |
| Catalog Eligibility | `No` |

## 3. 制約

- GB200 relation、procurement、deployment count、shipment又はrevenueを示さない。
