# Infineon Technologies — AI Demand-Supply Condition Raw Evidence
...
| Inspection Source | `InfineonSourceFactInspection.v0.1-draft.md` — `IFX-SFI-041` |
| Cross-sprint Recheck | `InfineonCrossSprintCandidateRecheck.v0.1-draft.md` — Accepted; no identical candidate |
| Evidence ID | `S3-EVR-129` |
| Evidence Registration Status | Registered — `Sprint003DataCenterAIInfrastructureEvidenceRegister.v0.1-draft.md` / `S3-EVR-129`; EVR Review Accepted |
| PIT Mapping Status | Registered — `Sprint003DataCenterAIInfrastructurePITInventory.v0.1-draft.md` / `S3-PIT-129`; PIT Review Accepted |

> **利用境界：** Product、customer、quantity、period、shortfall、backlog又はrevenue impactを補完しない。`allocation`又はcapacity redeploymentと別Factを維持する。 `AvailableAt = TBD — no use`、Catalog Eligibility `No`。
...
AI data-center businessではdemandがsupplyをstrongly exceedしていると発行者は説明する。

| Field | Value |
| --- | --- |
| Fact type | Management-reported demand / supply condition |
| Actual / Forecast | Management-reported demand / supply condition — 上流Inspectionの分類を変更しない |
| Unit | Unquantified demand / supply condition |
